...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app, **kwargs):
        ...
        self.limiters = {
            ...
            'strict': RateLimiter(
                rate=10,  # 10 requests per minute for sensitive endpoints
                burst=2,
                window=60
            ),
            'relaxed': RateLimiter(
                rate=300,  # 300 requests per minute for public endpoints
                burst=50,
                window=60
            )
        }
...
class APIKeyRateLimiter:
    ...
    def _cleanup_usage(self, api_key: str, now: float):
        ...
        for window, seconds in windows.items():
            cutoff = now - seconds
            self.usage[api_key][window] = [
                timestamp for timestamp in self.usage[api_key][window]
                if timestamp > cutoff
            ]
