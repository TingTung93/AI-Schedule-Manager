...
class ExportService:
    ...
    async def _export_ical(self, schedule_data: List[Dict]) -> bytes:
        ...
        for schedule in schedule_data:
            ...
            event.add('description', f"Employee: {schedule['Employee']}\n"
    ...
    async def _calculate_analytics(
        ...
        query = select(Schedule).join(Employee).join(Shift).where(
            and_(Schedule.date >= date_from, Schedule.date <= date_to)
        )
        result = await db.execute(query)
        schedules = result.scalars().all()
        ...
        total_hours = sum([
            (datetime.combine(date.today(), s.shift.end_time) -
             datetime.combine(date.today(), s.shift.start_time)).seconds / 3600
            for s in schedules
        ])
        ...
        for schedule in schedules:
            ...
            hours = (datetime.combine(date.today(), schedule.shift.end_time) -
                    datetime.combine(date.today(), schedule.shift.start_time)).seconds / 3600
