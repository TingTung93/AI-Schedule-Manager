...
class DataTransformer:
    ...
    def auto_map_columns(
        ...
        try:
            ...
            normalized_columns = {
                self._normalize_field_name(col): col for col in df_columns
            }
    ...
    def transform_employee_data(
        ...
        try:
            ...
            if 'hourly_rate' in transformed_df.columns:
                transformed_df['hourly_rate'] = transformed_df['hourly_rate'].apply(
                    self._clean_numeric_value
                )
            ...
            if 'qualifications' in transformed_df.columns:
                transformed_df['qualifications'] = transformed_df['qualifications'].apply(
    ...
    def transform_schedule_data(
        ...
        try:
            ...
            if 'employee_email' in transformed_df.columns:
                transformed_df['employee_email'] = transformed_df['employee_email'].apply(
                    self._clean_email
                )
            ...
            if 'status' in transformed_df.columns:
                transformed_df['status'] = transformed_df['status'].apply(
                    self._normalize_schedule_status
                )

            if 'overtime_approved' in transformed_df.columns:
                transformed_df['overtime_approved'] = transformed_df['overtime_approved'].apply(
                    self._parse_boolean
                )
    ...
    def transform_rule_data(
        ...
        try:
            ...
            if 'rule_type' in transformed_df.columns:
                transformed_df['rule_type'] = transformed_df['rule_type'].apply(
                    self._normalize_rule_type
                )

            if 'original_text' in transformed_df.columns:
                transformed_df['original_text'] = transformed_df['original_text'].apply(
                    self._clean_text
                )
            ...
            if 'constraints' in transformed_df.columns:
                transformed_df['constraints'] = transformed_df['constraints'].apply(
    ...
    def detect_data_types(self, df: pd.DataFrame) -> Dict[str, str]:
        ...
        for column in df.columns:
            ...
            # Check for boolean
            if all(str(val).lower() in self.boolean_true_values | self.boolean_false_values
