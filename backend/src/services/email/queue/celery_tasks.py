...
def send_email_task(
    ...
    try:
        ...
        try:
            ...
            return {"success": result["success"], "message_id": result.get("message_id"), "email_log_id": email_log.id}
...
def send_batch_emails_task(self, email_batch: List[Dict[str, Any]], batch_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        for email_data in email_batch:
            try:
                ...
                results.append({"email": email_data.get("to_email"), "task_id": task_result.id, "status": "queued"})
                success_count += 1

            except Exception as e:
                logger.error(f"Failed to queue email for {email_data.get('to_email')}: {e}")
                results.append({"email": email_data.get("to_email"), "status": "failed", "error": str(e)})
                failed_count += 1
...
def process_scheduled_emails() -> Dict[str, Any]:
    ...
    try:
        ...
        scheduled_emails = (
            ...
            .filter(EmailLog.scheduled_at <= now, EmailLog.status == EmailStatus.PENDING.value)
            .order_by(EmailLog.scheduled_at, EmailLog.priority.desc())
            .limit(100)
            .all()
        )
        ...
        return {"processed": processed_count, "message": f"Processed {processed_count} scheduled emails"}
...
def process_email_webhooks(webhook_data: Dict[str, Any], provider: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "processed_events": len(processed_events.get("events", [])), "provider": provider}
...
def update_bounce_list() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "new_bounces": new_count, "updated_bounces": updated_count}
