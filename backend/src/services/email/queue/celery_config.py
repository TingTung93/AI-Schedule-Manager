...
celery_app = Celery(
    ...
    include=[
        'src.services.email.queue.celery_tasks'
    ]
)
...
celery_app.conf.update(
    ...
    # Task routing
    task_routes={
        'src.services.email.queue.celery_tasks.send_email_task': {
            'queue': 'email_immediate'
        },
        'src.services.email.queue.celery_tasks.send_batch_emails_task': {
            'queue': 'email_batch'
        },
        'src.services.email.queue.celery_tasks.process_scheduled_emails': {
            'queue': 'email_scheduled'
        },
        'src.services.email.queue.celery_tasks.process_email_webhooks': {
            'queue': 'email_webhooks'
        },
        'src.services.email.queue.celery_tasks.cleanup_email_logs': {
            'queue': 'email_maintenance'
        }
    },
    ...
    task_annotations={
        ...
        'src.services.email.queue.celery_tasks.send_batch_emails_task': {
            'rate_limit': '10/m'   # 10 batch jobs per minute
        }
    },
