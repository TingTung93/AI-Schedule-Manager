    ...
    try:
        ...
        if result["success"]:
            return EmailResponse(
                success=True, task_id=result.get("task_id"), message=result.get("message", "Email queued successfully")
            )
        else:
            return EmailResponse(success=False, message="Failed to send email", error_message=result.get("error_message"))
...
async def send_templated_email(
    ...
    try:
        ...
        if result["success"]:
            ...
            return EmailResponse(
                success=False, message="Failed to send templated email", error_message=result.get("error_message")
            )
...
async def send_batch_emails(
    ...
    try:
        ...
        result = await email_service.send_batch_emails(emails=request.emails, batch_name=request.batch_name)
        ...
        if result["success"]:
            ...
            return EmailResponse(
                success=False, message="Failed to send batch emails", error_message=result.get("error_message")
            )
...
async def send_notification(
    ...
    try:
        ...
        if result["success"]:
            return EmailResponse(
                success=True, task_id=result.get("task_id"), message=result.get("message", "Notification queued successfully")
            )
        else:
            return EmailResponse(
                success=False, message="Failed to send notification", error_message=result.get("error_message")
            )
...
async def get_email_status(task_id: str, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
async def get_email_logs(
    ...
    try:
        ...
        return {"logs": [log.to_dict() for log in logs], "total": total, "limit": limit, "offset": offset}
...
async def send_test_email(
    to_email: EmailStr, test_type: str = "basic", db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
    """Send test email."""
    try:
        email_service = EmailService(db)
        ...
        if result["success"]:
            return {"success": True, "task_id": result.get("task_id"), "message": result.get("message")}
        else:
            return {"success": False, "message": "Failed to send test email", "error": result.get("error_message")}
...
async def verify_email(email: EmailStr, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    try:
        ...
        return {"email": email, "valid": is_valid, "timestamp": datetime.now(timezone.utc).isoformat()}
...
async def handle_webhook(provider: str, request: Request, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    try:
        ...
        background_tasks.add_task(email_service.process_webhook, payload, provider)

        return JSONResponse(status_code=200, content={"status": "webhook received"})
...
async def get_bounce_list(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    try:
        ...
        return {"bounces": bounce_list, "count": len(bounce_list), "timestamp": datetime.now(timezone.utc).isoformat()}
...
@require_permissions(["email:admin"])
async def remove_from_bounce_list(email: EmailStr, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    try:
        ...
        if success:
            ...
            return {"success": False, "message": f"Failed to remove {email} from bounce list"}
