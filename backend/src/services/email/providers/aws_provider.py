...
class AWSProvider(BaseEmailProvider):
    ...
    async def send_email(self, message: EmailMessage) -> EmailResponse:
        ...
        try:
            ...
            email_data = {
                ...
                "Message": {"Subject": {"Data": message.subject, "Charset": "UTF-8"}, "Body": {}},
            }
            ...
            if message.text_content:
                email_data["Message"]["Body"]["Text"] = {"Data": message.text_content, "Charset": "UTF-8"}

            if message.html_content:
                email_data["Message"]["Body"]["Html"] = {"Data": message.html_content, "Charset": "UTF-8"}
            ...
            if headers:
                email_data["Tags"] = [{"Name": key, "Value": value} for key, value in headers.items()]

            # Send email
            response = await asyncio.get_event_loop().run_in_executor(None, self.client.send_email, **email_data)

            return EmailResponse(success=True, message_id=response["MessageId"], provider_response=response, status_code=200)
            ...
            return EmailResponse(success=False, error_message=f"AWS SES error: {str(e)}")
    ...
    async def verify_email(self, email: str) -> bool:
        ...
        try:
            response = await asyncio.get_event_loop().run_in_executor(
                None, self.client.get_identity_verification_attributes, {"Identities": [email]}
            )
    ...
    async def get_bounce_list(self) -> List[Dict[str, Any]]:
        ...
        try:
            response = await asyncio.get_event_loop().run_in_executor(None, self.client.list_suppressed_destinations)
    ...
    async def remove_from_bounce_list(self, email: str) -> bool:
        ...
        try:
            await asyncio.get_event_loop().run_in_executor(
                None, self.client.delete_suppressed_destination, {"EmailAddress": email}
            )
            return True
    ...
    async def get_sending_quota(self) -> Dict[str, Any]:
        ...
        try:
            response = await asyncio.get_event_loop().run_in_executor(None, self.client.get_send_quota)
            return response
        except Exception as e:
    ...
    async def get_sending_rate(self) -> Dict[str, Any]:
        ...
        try:
            response = await asyncio.get_event_loop().run_in_executor(None, self.client.get_send_statistics)
            return response
        except Exception as e:
