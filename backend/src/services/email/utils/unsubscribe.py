...
class UnsubscribeManager:
    ...
    def generate_unsubscribe_token(self, user_id: int) -> str:
        ...
        try:
            ...
            preferences = self.db_session.query(NotificationPreference).filter_by(user_id=user_id).first()
    ...
    def process_unsubscribe(
        self, token: str, email_type: Optional[str] = None, reason: Optional[str] = None
    ) -> Dict[str, Any]:
        """Process unsubscribe request."""
        try:
            # Find user by token
            preferences = self.db_session.query(NotificationPreference).filter_by(unsubscribe_token=token).first()
    ...
    def _unsubscribe_from_type(self, preferences: NotificationPreference, email_type: str) -> Dict[str, Any]:
        ...
        return {"success": True, "message": f"Unsubscribed from {email_type} emails", "email_type": email_type}
    ...
    def _unsubscribe_from_all(self, preferences: NotificationPreference) -> Dict[str, Any]:
        ...
        return {"success": True, "message": "Unsubscribed from all emails", "email_type": "all"}

    def resubscribe(self, token: str, email_type: Optional[str] = None) -> Dict[str, Any]:
    ...
    def _resubscribe_to_type(self, preferences: NotificationPreference, email_type: str) -> Dict[str, Any]:
        ...
        return {"success": True, "message": f"Resubscribed to {email_type} emails", "email_type": email_type}
    ...
    def _resubscribe_to_all(self, preferences: NotificationPreference) -> Dict[str, Any]:
        ...
        return {"success": True, "message": "Resubscribed to all emails", "email_type": "all"}
    ...
    def get_preference_by_token(self, token: str) -> Optional[NotificationPreference]:
        ...
        try:
            return self.db_session.query(NotificationPreference).filter_by(unsubscribe_token=token).first()

        except Exception as e:
            logger.error(f"Get preferences by token error: {e}")
            return None

    def update_preferences(self, token: str, preferences_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            allowed_fields = [
                ...
                "system_alerts",
                "marketing_emails",
                "schedule_notification_frequency",
                "summary_frequency",
                "reminder_frequency",
                "quiet_hours_start",
                "quiet_hours_end",
            ...
            if updated_fields:
                ...
                return {"success": True, "message": "Preferences updated successfully", "updated_fields": updated_fields}
            else:
    ...
    def get_unsubscribe_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            unsubscribed_users = (
                self.db_session.query(NotificationPreference)
                .filter(NotificationPreference.unsubscribed_at.isnot(None))
                .count()
            )

            disabled_email_users = self.db_session.query(NotificationPreference).filter_by(email_enabled=False).count()
            ...
            for notification_type in notification_types:
                disabled_count = (
                    self.db_session.query(NotificationPreference)
                    .filter(getattr(NotificationPreference, notification_type) == False)
                    .count()
                )
