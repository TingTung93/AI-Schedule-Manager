...
class ImportService:
    ...
    async def _process_employee_import(
        self,
        db: AsyncSession,
        df: pd.DataFrame,
                ...
                if existing_employee:
                    if options.get('update_existing', False):
                        ...
                        results['errors'].append({
                            'row': index + 1,
                            'error': f"Employee with email {employee_data['email']} already exists"
                        })
                        continue
                else:
                    # Create new employee
    ...
    async def _process_schedule_import(
        self,
        db: AsyncSession,
        df: pd.DataFrame,
        options: Dict[str, Any]
            ...
            try:
                ...
                if shift_name not in shifts_cache:
                    ...
                    if not shift:
                        results['errors'].append({
                            'row': index + 1,
                            'error': f"Shift with name {shift_name} not found"
                        })
                        continue
                ...
                if existing_schedule:
                    if options.get('update_existing', False):
                        ...
                        update_data = {k: v for k, v in schedule_data.items()
                                     if k not in ['employee_id', 'shift_id', 'date']}
                        ...
                        results['errors'].append({
                            'row': index + 1,
                            'error': f"Schedule already exists for {email} on {schedule_date}"
                        })
                        continue
                else:
                    # Create new schedule
    ...
    async def _process_rule_import(
        self,
        db: AsyncSession,
        df: pd.DataFrame,
        options: Dict[str, Any]
        ...
        if import_type == 'employees':
            ...
            for email in internal_dups:
                ...
                duplicates['internal_duplicates'].append({
                    'field': 'email',
                    'value': email,
                    'rows': [r + 1 for r in dup_rows]  # 1-based indexing
                })
