...
class EventHandler:
    ...
    async def handle_event(connection_id: str, event_type: str, data: Dict[str, Any]):
        ...
        try:
            ...
            await manager.send_message(
                connection_id, {"type": "error", "message": f"Error processing event: {str(e)}", "event_type": event_type}
            )
    ...
    async def handle_join_room(connection_id: str, data: Dict[str, Any]):
        ...
        room = data.get("room")
        if not room:
            await manager.send_message(connection_id, {"type": "error", "message": "Room name required"})
            return

        success = await manager.join_room(connection_id, room)
        await manager.send_message(connection_id, {"type": "room_joined" if success else "room_join_failed", "room": room})
    ...
    async def handle_leave_room(connection_id: str, data: Dict[str, Any]):
        ...
        await manager.send_message(connection_id, {"type": "room_left", "room": room})
    ...
    async def handle_presence_event(connection_id: str, event_type: str, data: Dict[str, Any]):
        ...
        await manager.broadcast_to_room(
            "presence",
            {"type": event_type, "connection_id": connection_id, "timestamp": datetime.now().isoformat(), **data},
...
class NotificationEvents:
    ...
    async def new_notification(user_id: int, notification_data: Dict[str, Any]):
        ...
        await manager.send_personal_message(
            {"type": EventType.NOTIFICATION_NEW, "data": notification_data, "timestamp": datetime.now().isoformat()}, user_id
        )
...
def format_event_message(event_type: str, data: Dict[str, Any], **kwargs) -> Dict[str, Any]:
    ...
    return {"type": event_type, "data": data, "timestamp": datetime.now().isoformat(), **kwargs}


def format_error_message(error: str, code: Optional[str] = None) -> Dict[str, Any]:
    """Format an error message"""
    return {"type": "error", "error": error, "code": code, "timestamp": datetime.now().isoformat()}
