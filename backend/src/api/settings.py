...
DEFAULT_SETTINGS = {
    "notifications": {"email": True, "push": False, "scheduleReminders": True, "overtimeAlerts": True},
    "appearance": {"theme": "light", "language": "en", "timezone": "America/New_York"},
    "scheduling": {"defaultShiftLength": 8, "maxOvertimeHours": 10, "breakDuration": 30, "autoApproveRequests": False},
    "security": {"twoFactorAuth": False, "sessionTimeout": 60},
}


@router.get("", response_model=UserSettingsResponse)
async def get_settings(db: AsyncSession = Depends(get_database_session), current_user: dict = Depends(get_current_user)):
