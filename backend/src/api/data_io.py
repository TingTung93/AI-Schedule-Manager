...
async def export_employees(
    ...
    try:
        ...
        content = await export_service.export_employees(
            db=db, format_type=format_type, filters=filters, include_inactive=include_inactive
        )
...
async def export_schedules(
    ...
    try:
        ...
        content = await export_service.export_schedules(
            db=db, format_type=format_type, date_from=date_from, date_to=date_to, employee_ids=employee_ids, filters=filters
        )
        ...
        extensions = {"csv": "csv", "excel": "xlsx", "pdf": "pdf", "ical": "ics"}
...
async def export_analytics_report(
    ...
    try:
        content = await export_service.export_analytics_report(
            db=db, format_type=format_type, date_from=date_from, date_to=date_to
        )
...
async def upload_import_file(file: UploadFile = File(...), current_user: dict = Depends(get_current_manager)):
    """Upload file for import processing."""
    try:
        file_info = await file_handler.validate_and_process_upload(
            file=file, allowed_extensions=[".csv", ".xlsx", ".xls"], max_size=50 * 1024 * 1024  # 50MB
        )
...
async def preview_import_data(
    ...
    try:
        ...
        preview = await import_service.preview_import_data(
            file_content=file_content, filename=file_info["filename"], import_type=import_type, preview_rows=preview_rows
        )
...
async def validate_import_data(
    ...
    try:
        ...
        df = await import_service._read_file_to_dataframe(file_content, file_info["filename"], "utf-8")
...
async def execute_import(
    ...
    try:
        ...
        options = {"update_existing": update_existing, "column_mapping": column_mapping or {}}
...
async def get_import_progress(file_id: str, current_user: dict = Depends(get_current_user)):
...
async def create_backup(
    ...
    try:
        backup_id = await backup_service.create_backup(
            db=db, backup_type=backup_type, include_files=include_files, user_id=current_user.get("user_id", "unknown")
        )
...
async def list_backups(
    ...
    try:
        backups = await backup_service.list_backups(skip=(page - 1) * size, limit=size)

        return PaginatedResponse(
            items=backups["items"], total=backups["total"], page=page, size=size, pages=(backups["total"] + size - 1) // size
        )
...
async def get_backup_status(backup_id: str, current_user: dict = Depends(get_current_manager)):
...
async def delete_backup(backup_id: str, current_user: dict = Depends(get_current_manager)):
    """Delete a backup."""
    try:
...
async def cleanup_expired_files(background_tasks: BackgroundTasks, current_user: dict = Depends(get_current_manager)):
