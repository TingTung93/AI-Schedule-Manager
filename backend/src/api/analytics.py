...
async def get_labor_costs(
    ...
    result = await db.execute(
        select(Shift.date, func.sum(func.extract("epoch", Shift.end_time - Shift.start_time) / 3600).label("total_hours"))
        .select_from(ScheduleAssignment)
        .join(Shift, ScheduleAssignment.shift_id == Shift.id)
        .where(
            and_(
    ...
    for i in range(days):
        ...
        data.append({"date": current_date.isoformat(), "cost": round(cost, 2), "hours": round(hours, 2)})

    total_cost = sum(d["cost"] for d in data)
    average_cost = total_cost / len(data) if data else 0

    return {"data": data, "total": round(total_cost, 2), "average": round(average_cost, 2)}
...
async def get_performance_metrics(
    ...
    # Total assignments
    total_assignments = await db.scalar(select(func.count(ScheduleAssignment.id))) or 1

    # Completed assignments
    completed_assignments = (
        await db.scalar(select(func.count(ScheduleAssignment.id)).where(ScheduleAssignment.status == "completed")) or 0
    )
    ...
    declined_assignments = (
        await db.scalar(select(func.count(ScheduleAssignment.id)).where(ScheduleAssignment.status == "declined")) or 0
    )
...
async def get_efficiency_metrics(
    ...
    # Count total shifts
    total_shifts = await db.scalar(select(func.count(Shift.id))) or 1

    # Count shifts with at least one assignment
    assigned_shifts = (
        await db.scalar(
            select(func.count(func.distinct(ScheduleAssignment.shift_id))).where(
                ScheduleAssignment.status.in_(["assigned", "confirmed", "completed"])
            )
        )
        or 0
    ...
    fully_staffed_result = await db.execute(
        select(Shift.id, Shift.required_staff, func.count(ScheduleAssignment.id).label("assigned_count"))
        .outerjoin(
            ScheduleAssignment,
            and_(
                ScheduleAssignment.shift_id == Shift.id, ScheduleAssignment.status.in_(["assigned", "confirmed", "completed"])
            ),
        )
