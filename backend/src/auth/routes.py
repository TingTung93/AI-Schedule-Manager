...
def log_audit_event(event_type: str, user_id: int = None, resource: str = None,
                   action: str = None, success: bool = True, failure_reason: str = None,
                   details: dict = None):
...
def record_login_attempt(email: str, user_id: int = None, success: bool = True,
...
def register():
    ...
    try:
        ...
        if existing_user:
            session.close()
            log_audit_event('registration', resource='user', action='create',
        ...
        log_audit_event('registration', user_id=new_user.id, resource='user',
...
def login():
    ...
    try:
        ...
        if user.is_account_locked():
            record_login_attempt(email, user_id=user.id, success=False,
        ...
        if not auth_service.verify_password(password, user.password_hash):
            ...
            if user.failed_login_attempts >= 5:
                ...
                log_audit_event('account_locked', user_id=user.id, resource='user',
                               action='lock', success=True,
        ...
        log_audit_event('login', user_id=user.id, resource='user', action='login',
                       success=True)
...
def refresh_token():
    ...
    try:
        ...
        # Log token refresh
        log_audit_event('token_refresh', user_id=user_id, resource='auth',
...
def logout():
    ...
    try:
        ...
        # Log logout
        log_audit_event('logout', user_id=g.user_id, resource='user',
...
def change_password():
    ...
    try:
        ...
        if not auth_service.verify_password(current_password, user.password_hash):
            session.close()
            log_audit_event('password_change', user_id=g.user_id, resource='user',
                           action='change_password', success=False,
        ...
        log_audit_event('password_change', user_id=g.user_id, resource='user',
                       action='change_password', success=True)
...
def forgot_password():
    ...
    try:
        ...
        if user and user.is_active:
            ...
            log_audit_event('password_reset_request', user_id=user.id,
                           resource='user', action='reset_request', success=True)
...
def reset_password():
    ...
    try:
        ...
        log_audit_event('password_reset', user_id=user_id, resource='user',
...
def revoke_session(token_jti):
    ...
    try:
        ...
        log_audit_event('session_revoke', user_id=g.user_id, resource='auth',
                       action='revoke_session', success=True,
