...
class Schedule(Base):
    ...
    parent_schedule: Mapped[Optional["Schedule"]] = relationship(
        "Schedule",
        remote_side=[id],
        back_populates="child_schedules"
    )

    child_schedules: Mapped[List["Schedule"]] = relationship(
        "Schedule",
        back_populates="parent_schedule"
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "week_start <= week_end",
            name="valid_week_period"
        ),
        CheckConstraint(
            "week_end - week_start <= INTERVAL '7 days'",
            name="max_week_duration"
        ),
        CheckConstraint(
            "status IN ('draft', 'pending_approval', 'approved', 'published', 'archived', 'rejected')",
            name="valid_status"
        ),
        CheckConstraint(
            "version > 0",
    ...
    def get_assigned_employees(self) -> List["Employee"]:
        ...
        return list(set(assignment.employee for assignment in self.assignments
                       if assignment.status == "assigned"))

    def get_unassigned_shifts(self) -> List["Shift"]:
        """Get shifts that still need staff assignments"""
        assigned_shifts = {assignment.shift_id for assignment in self.assignments
                          if assignment.status == "assigned"}
    ...
    def get_coverage_summary(self) -> dict:
        ...
        assigned_shifts = len(set(assignment.shift_id for assignment in self.assignments
                                 if assignment.status == "assigned"))
