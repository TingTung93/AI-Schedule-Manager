...
from src.scheduler.constraint_solver import (
    ScheduleOptimizer,
    ShiftType,
    Employee,
    Shift,
...
class TestScheduleOptimizer:
    ...
    def test_shift_hours_calculation(self, optimizer):
        ...
        employees = [Employee(
            id="emp1", name="Test", role="Test", 
            min_hours_week=0, max_hours_week=40,
            hourly_rate=15.0, skills=[], availability={}
        )]
        
        shifts = [
            Shift(id="day", day=0, start_hour=9, end_hour=17,
                  required_role="", required_skills=[], min_staff=1, max_staff=1),
            Shift(id="overnight", day=0, start_hour=22, end_hour=6,
                  required_role="", required_skills=[], min_staff=1, max_staff=1)
        ]
    ...
    def test_has_required_qualifications(self, optimizer):
        ...
        employee = Employee(
            id="emp1", name="Test", role="Cashier",
            min_hours_week=0, max_hours_week=40,
            hourly_rate=15.0, skills=["cash_handling", "inventory"],
            availability={}
        )
        
        # Matching role and skills
        shift1 = Shift(id="s1", day=0, start_hour=9, end_hour=17,
                      required_role="Cashier", required_skills=["cash_handling"],
                      min_staff=1, max_staff=1)
        assert optimizer._has_required_qualifications(employee, shift1) is True
        
        # Wrong role
        shift2 = Shift(id="s2", day=0, start_hour=9, end_hour=17,
                      required_role="Manager", required_skills=[],
                      min_staff=1, max_staff=1)
        assert optimizer._has_required_qualifications(employee, shift2) is False
        
        # Missing skill
        shift3 = Shift(id="s3", day=0, start_hour=9, end_hour=17,
                      required_role="Cashier", required_skills=["customer_service"],
                      min_staff=1, max_staff=1)
    ...
    @patch('src.scheduler.constraint_solver.cp_model.CpSolver')
    def test_generate_schedule_infeasible(self, mock_solver_class, optimizer,
                                         sample_employees, sample_shifts, sample_constraints):
    ...
    def test_add_rest_period_constraints(self, optimizer, sample_employees):
        ...
        shifts = [
            Shift(id="late_shift", day=0, start_hour=16, end_hour=23,
                  required_role="", required_skills=[], min_staff=1, max_staff=1),
            Shift(id="early_shift", day=1, start_hour=6, end_hour=14,
                  required_role="", required_skills=[], min_staff=1, max_staff=1)
        ]
    ...
    def test_extract_solution(self, optimizer, sample_employees, sample_shifts):
        ...
        result = optimizer._extract_solution(
            sample_employees,
            sample_shifts,
            date(2024, 1, 1),
            cp_model.OPTIMAL
    ...
    def test_generate_fallback_schedule(self, optimizer, sample_employees, sample_shifts):
        ...
        result = optimizer._generate_fallback_schedule(
            sample_employees,
            sample_shifts,
            date(2024, 1, 1)
        )
    ...
    def test_edge_case_no_employees(self, optimizer):
        ...
        result = optimizer.generate_schedule(
            [],  # No employees
            [Shift(id="s1", day=0, start_hour=9, end_hour=17,
                  required_role="", required_skills=[], min_staff=1, max_staff=1)],
            [],
            date(2024, 1, 1)
        )
