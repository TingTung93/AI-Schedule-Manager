...
class TestConstraintSolver:
    ...
    def sample_constraints(self):
        ...
        return [
            Constraint(
                ...
                parameters={"employee_id": "emp-1", "day_of_week": 0, "start_time": "09:00", "end_time": "17:00"},  # Monday
            ),
            Constraint(
                id="constraint-2", constraint_type="max_hours", parameters={"employee_id": "emp-1", "max_hours_day": 8}
            ),
            Constraint(
                id="constraint-3",
                constraint_type="skill_requirement",
                parameters={"position": "Cook", "required_skills": ["cooking", "food_safety"]},
            ),
        ]
    ...
    def test_basic_schedule_solving(self, solver, sample_employees, sample_shifts):
        ...
        solution = solver.solve(employees=sample_employees, shifts=sample_shifts, constraints=[])
    ...
    def test_availability_constraint_enforcement(self, solver, sample_employees, sample_shifts):
        ...
        solution = solver.solve(employees=sample_employees, shifts=sample_shifts, constraints=[availability_constraint])

        # Verify emp-1 is not assigned to evening shifts
        for assignment in solution["assignments"]:
            if assignment["employee_id"] == "emp-1" and assignment["shift_id"] == "shift-2":  # Evening shift
    ...
    def test_max_hours_constraint(self, solver, sample_employees):
        ...
        max_hours_constraint = Constraint(
            constraint_type="max_hours_week", parameters={"employee_id": "emp-1", "max_hours": 40}
        )

        solution = solver.solve(employees=sample_employees, shifts=long_shifts, constraints=[max_hours_constraint])
    ...
    def test_skill_requirement_constraint(self, solver, sample_employees):
        ...
        skill_constraint = Constraint(
            constraint_type="skill_requirement", parameters={"position": "Cook", "required_skills": ["cooking", "food_safety"]}
        )
        ...
        solution = solver.solve(employees=sample_employees, shifts=[cooking_shift], constraints=[skill_constraint])

        # Verify only qualified employee is assigned
        for assignment in solution["assignments"]:
            if assignment["shift_id"] == "cook-shift-1":
                assigned_employee = next(e for e in sample_employees if e.id == assignment["employee_id"])
    ...
    def test_minimum_staffing_constraint(self, solver, sample_employees):
        ...
        min_staffing_constraint = Constraint(
            constraint_type="min_staffing", parameters={"shift_id": "busy-shift", "min_employees": 3}
        )

        solution = solver.solve(employees=sample_employees, shifts=[understaffed_shift], constraints=[min_staffing_constraint])

        # Count assignments for this shift
        shift_assignments = [a for a in solution["assignments"] if a["shift_id"] == "busy-shift"]
    ...
    def test_consecutive_shift_constraint(self, solver, sample_employees):
        ...
        no_consecutive_constraint = Constraint(constraint_type="no_consecutive_shifts", parameters={"min_break_hours": 8})

        solution = solver.solve(employees=sample_employees, shifts=[shift1, shift2], constraints=[no_consecutive_constraint])

        # Check that no employee works both shifts
        morning_employees = {a["employee_id"] for a in solution["assignments"] if a["shift_id"] == "shift-morning"}
        afternoon_employees = {a["employee_id"] for a in solution["assignments"] if a["shift_id"] == "shift-afternoon"}
    ...
    def test_fair_distribution_optimization(self, solver, sample_employees):
        ...
        fair_distribution_constraint = Constraint(
            constraint_type="fair_distribution", parameters={"variance_threshold": 0.2}  # Max 20% variance in hours
        )

        solution = solver.solve(employees=sample_employees, shifts=shifts, constraints=[fair_distribution_constraint])
    ...
    def test_solver_performance_with_large_input(self, solver):
        ...
        solution = solver.solve(
            employees=employees, shifts=shifts, constraints=[], timeout_seconds=30  # Give reasonable time limit
        )
        end_time = time.time()
    ...
    def test_infeasible_schedule_detection(self, solver, sample_employees):
        ...
        solution = solver.solve(employees=sample_employees, shifts=[impossible_shift], constraints=[])
    ...
    def test_solution_validation(self, solver, sample_employees, sample_shifts, sample_constraints):
        ...
        solution = solver.solve(employees=sample_employees, shifts=sample_shifts, constraints=sample_constraints)

        is_valid, violations = solver.validate_solution(solution, sample_constraints)
        ...
        invalid_solution = {
            "assignments": [
                {"employee_id": "emp-1", "shift_id": "shift-1", "start_time": "09:00", "end_time": "25:00"}  # Invalid time
            ],
            "status": "test",
        }

        is_valid, violations = solver.validate_solution(invalid_solution, sample_constraints)
        assert not is_valid
...
class TestRuleParser:
    ...
    def test_parse_invalid_rule(self, parser):
        ...
        invalid_rules = ["", "random text with no meaning", "maybe sometimes perhaps", "!@#$%^&*()"]  # Empty rule
...
class TestNLPOptimizer:
    ...
    def sample_schedule_data(self):
        ...
        return {
            ...
            "shifts": [
                {"id": "shift-1", "date": "2024-01-15", "start_time": "09:00", "end_time": "17:00", "position": "Server"}
            ],
            "assignments": [{"employee_id": "emp-1", "shift_id": "shift-1"}],
        }
    ...
    def test_cost_optimization(self, optimizer, sample_schedule_data):
        ...
        result = optimizer.optimize(schedule_data=sample_schedule_data, optimization_goals=["minimize_cost"])
    ...
    def test_coverage_optimization(self, optimizer, sample_schedule_data):
        ...
        result = optimizer.optimize(schedule_data=sample_schedule_data, optimization_goals=["maximize_coverage"])
    ...
    def test_fairness_optimization(self, optimizer, sample_schedule_data):
        ...
        result = optimizer.optimize(schedule_data=sample_schedule_data, optimization_goals=["improve_fairness"])
    ...
    def test_multi_objective_optimization(self, optimizer, sample_schedule_data):
        ...
        result = optimizer.optimize(
            schedule_data=sample_schedule_data, optimization_goals=["minimize_cost", "maximize_coverage", "improve_fairness"]
        )
    ...
    def test_constraint_satisfaction_during_optimization(self, optimizer, sample_schedule_data):
        ...
        constraints = [{"type": "max_hours", "employee_id": "emp-1", "max_hours_week": 30}]

        result = optimizer.optimize(
            schedule_data=sample_schedule_data, optimization_goals=["minimize_cost"], constraints=constraints
        )
    ...
    def test_optimization_with_preferences(self, optimizer, sample_schedule_data):
        ...
        result = optimizer.optimize(
            schedule_data=sample_schedule_data, optimization_goals=["maximize_satisfaction"], preferences=preferences
        )
...
class TestBusinessLogicIntegration:
    ...
    def test_end_to_end_schedule_generation(self):
        ...
        for parsed_rule in parsed_rules:
            if parsed_rule.rule_type == "availability":
                constraints.append(
                    Constraint(constraint_type="availability", parameters={"employee_name": parsed_rule.employee_name})
                )
        ...
        shifts = [
            Shift(id="lunch-shift", date=date(2024, 1, 15), start_time=time(12, 0), end_time=time(14, 0), required_employees=2)
        ]
        ...
        optimized = optimizer.optimize(schedule_data=solution, optimization_goals=["minimize_cost", "maximize_coverage"])
    ...
    def test_performance_with_realistic_data_size(self):
        ...
        # Create test data
        employees = [
            Employee(id=f"emp-{i}", name=f"Employee {i}", email=f"emp{i}@example.com", role="Staff") for i in range(25)
        ]
        ...
        constraints = [Constraint(constraint_type="max_hours_week", parameters={"max_hours": 40}) for _ in range(20)]
